...
public class paymentAnalysisFinalReducer extends TableReducer<LongWritable, profileWritable, ImmutableBytesWritable> {
	public final byte[] family = "d".getBytes();
	public final byte[] qualifier = "pay".getBytes();
	...
	public void reduce(LongWritable key, Iterable<profileWritable> values, Context context) throws IOException, InterruptedException {
		...
		if(userProfile && sourceProfile) {
			...
			String rowKey = String.format("sw-%s", String.valueOf(result));
			Put put = new Put(rowKey.getBytes());
			put.add(family, qualifier, String.valueOf(result).getBytes());
